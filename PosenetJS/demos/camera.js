...
//check
let count_sum = 0;
let count_sum_for_left = 0;
let count_sum_for_right = 0;
let count_sum_for_distance = 0;
  ...
  async function poseDetectionFrame() {
    ...
    poses.forEach(({ score, keypoints }) => {
      ...
      if (getStableKeyPoints(1, "x") == null) {
        ...
        eyesDirection.x += 0.00001
        let EyesSlopeRate = eyesDirection.y / eyesDirection.x
        document.getElementById('EyesSlope').value = EyesSlopeRate;
        textToDisplayForDebug += "\nCurrent Eyes Slope = " + EyesSlopeRate;

        //range: abs(slope)<0.1310 
        if (Math.abs(EyesSlopeRate) > 0.1311) {
          count_sum++;
          if (EyesSlopeRate > 0) {
            count_sum_for_right++;
            ErrorMsg += " 頭有點右斜喔(眼鼻太右) \n";
          } else if (EyesSlopeRate < 0) {
            count_sum_for_left++;
            ErrorMsg += " 頭有點左斜喔(眼鼻太左) \n";
          }
        }
        ...
        if (count_sum_for_left > 200) {//1000
          document.getElementById('count_sum_for_left').value = count_sum_for_left;


          if (document.getElementById('left_alert').value == "left") {
            document.getElementById('left_alert').value = "";
            count_sum_for_left = 0;
          }
        }
        //電腦端歪右邊提醒
        if (count_sum_for_right > 200) {//1000
          document.getElementById('count_sum_for_right').value = count_sum_for_right;

          if (document.getElementById('right_alert').value == "right") {
            document.getElementById('right_alert').value = "";
            count_sum_for_right = 0;
          }
        }
      }
      // 取得耳朵的點
      if (getStableKeyPoints(3, "x") == null) {
        textToDisplayForDebug = "Frames is not accmulated enough to do median stable selection";
      } else {
        let earsDirection = {
          "x": getStableKeyPoints(3, "x") - getStableKeyPoints(4, "x"),
          "y": getStableKeyPoints(3, "y") - getStableKeyPoints(4, "y"),
        }

        // 算斜率
        // Increase the x a bit to avoid the chance of divided by 0
        earsDirection.x += 0.00001
        let EarsSlopeRate = earsDirection.y / earsDirection.x
        document.getElementById('EarsSlope').value = EarsSlopeRate;
        textToDisplayForDebug += "\nCurrent Ears Slope = " + EarsSlopeRate;

        //range: abs(slope)<0.1382
        if (Math.abs(EarsSlopeRate) > 0.1383) {
          count_sum++;
          if (EarsSlopeRate > 0) {
            count_sum_for_right++;
            ErrorMsg += " 頭有點右斜喔(眼耳太右) \n";
          } else if (EarsSlopeRate < 0) {
            count_sum_for_left++;
            ErrorMsg += " 頭有點左斜喔(眼耳太左) \n";
          }
        }
      ...
      if (getStableKeyPoints(0, "x") == null) {
        ...
        //range: area<1812.3617
        if (area > 1812.3617) {
          count_sum++;
          count_sum_for_distance++;
          ErrorMsg += " 太靠近螢幕囉!(鼻眼太近) ";
        }
        ...
        //電腦端太近提醒
        if (count_sum_for_distance > 100) {//500
          document.getElementById('count_sum_for_distance').value = count_sum_for_distance;

          if (document.getElementById('distance_alert').value == "distance") {
            document.getElementById('distance_alert').value = "";
            count_sum_for_distance = 0;
          }
